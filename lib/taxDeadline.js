...
export function chuanHoaKy(kyKKhai, periodKind) {
  ...
  // Bảng tra cứu ghi sẵn dạng 'Q2/2026' hoặc 'T09/2026' — nhận luôn, khỏi phải đoán theo loại kỳ.
  const coTien = s.match(/^([QT])\s*(\d{1,2})\s*\/\s*(\d{4})$/i)
  if (coTien) {
    const so = +coTien[2]
    return coTien[1].toUpperCase() === 'Q'
      ? (so >= 1 && so <= 4 ? `Q${so}.${coTien[3]}` : null)
      : (so >= 1 && so <= 12 ? `T${HAI_CHU_SO(so)}.${coTien[3]}` : null)
  }
