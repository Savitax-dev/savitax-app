...
// Tình trạng khách 7 bước (chốt 2026-09-21, sql/15_sales_stage_7.sql): Đang chăm sóc → Gửi khảo sát →
// Gửi báo giá → Chốt báo giá → Gửi hợp đồng → Chốt hợp đồng | Thất bại. Mã giữ tên cũ cho đỡ đổi dữ liệu:
// tu_van = Đang chăm sóc, bao_gia = Gửi báo giá, chot = Chốt hợp đồng. 'moi' đã gộp vào tu_van (còn nhận
// để không lỗi với dữ liệu cũ). Tự đẩy chỉ đi TIẾN; 'that_bai' đứng ngoài chuỗi.
export const STAGE_ORDER = ['moi', 'tu_van', 'gui_khao_sat', 'bao_gia', 'chot_bao_gia', 'gui_hd', 'chot']
export const STAGE_LABEL = {
  moi: 'Đang chăm sóc', tu_van: 'Đang chăm sóc', gui_khao_sat: 'Gửi khảo sát', bao_gia: 'Gửi báo giá',
  chot_bao_gia: 'Chốt báo giá', gui_hd: 'Gửi hợp đồng', chot: 'Chốt hợp đồng', that_bai: 'Thất bại',
}
// Trạng thái HĐ của báo giá đi kèm từng tình trạng khách khi đổi ở danh sách báo giá.
export function contractForStage(stage, current) {
  if (stage === 'gui_hd') return 'sent'
  if (stage === 'chot') return 'signed'
  if (stage === 'that_bai') return current === 'signed' ? 'sent' : current
  return 'draft'
}
