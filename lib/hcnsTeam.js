...
export async function getHcnsTeam(supabase) {
  ...
  // Người KIÊM NHIỆM phòng HCNS (staff_extra_roles) — họ giữ phòng chính bên kế toán nên không
  // lọt vào 2 truy vấn trên. Bảng chưa cài thì bỏ qua.
  let byExtra = []
  if (room) {
    const { data: extra } = await supabase.from('staff_extra_roles')
      .select('staff_id').eq('room_id', room.id)
    const ids = [...new Set((extra || []).map(e => e.staff_id))]
    if (ids.length) {
      byExtra = (await supabase.from('staff').select('id, full_name, role, room_id')
        .in('id', ids).eq('is_active', true)).data || []
    }
  }

  const map = new Map()
  for (const s of [...byRole, ...byRoom, ...byExtra]) map.set(s.id, s)
