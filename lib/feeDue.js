...
import { startedByMonth, countsForMonth } from './contractDates.js'
...
export function dueFeeMonthsCount(client, year, months, now = new Date()) {
  return months.filter(m => countsForMonth(client, year, m) && feeCountsForMonth(client.fee_period, year, m, now)).length
}
