...
export function evaluateCap({ amount, fee, reliable, year, month, paidPeriods = new Set(), label = 'phí', otherDebt = 0 }) {
  const amt = Number(amount) || 0
  const f = Number(fee) || 0
  const debt = Number(otherDebt) || 0
  ...
  // CHỈ cho trả gộp khi công ty KHÔNG còn nợ tồn (quy tắc chốt 2026-09-09). Công ty đang có nợ
  // tồn thì số khách trả hiếm khi tròn đúng bội số phí kỳ — gộp lúc đó dễ ghi tiền của nợ tồn
  // thành tiền phí tháng, và luồng gộp cũng không trừ nợ tồn nên sẽ tính trùng. Có nợ tồn thì
  // luôn đi hướng EXCESS: ghi đúng phí kỳ, phần dư trừ thẳng vào nợ tồn.
  if (isCleanMultiple && debt <= 0) {
    return {
      kind: CAP_SPLIT,
      periods: rounded,
      perPeriod: f,
  ...
  return {
    ...
    message: 'Số tiền vượt quá ' + label + ' ' + periodLabel + ' (' + fmt(f) + 'đ). '
      + (debt > 0
        ? 'Công ty đang còn nợ tồn ' + fmt(debt) + 'đ nên không gộp kỳ được — ghi ' + fmt(f)
          + 'đ cho kỳ này, phần dư ' + fmt(amt - f) + 'đ trừ thẳng vào nợ tồn.'
        : 'Ghi ' + fmt(f) + 'đ ở mục này, phần dư ' + fmt(amt - f) + 'đ chuyển sang mục "Nợ tồn cũ".'),
  }
}

// Gợi ý N kỳ để rải đều: ưu tiên các kỳ CHƯA có ghi nhận thu, tính TỚI TRƯỚC từ kỳ đang nhập
// (đổi 2026-09-09 theo yêu cầu: trả gộp ở T8 nghĩa là đóng trước phí T8 và T9). Chỉ khi các
// tháng sau không đủ mới lấy lùi về trước.
//
// Trước đây ưu tiên LÙI (trả bù tháng nợ) — nay không cần nữa vì trả gộp chỉ áp dụng cho công
// ty KHÔNG còn nợ tồn (xem evaluateCap), tức là không có tháng cũ nào đang treo để phải bù.
