...
// Phí HCNS theo đầu người: 200.000 → 300.000 đ/người/tháng (người dùng chốt 2026-09-21). Báo giá đã lưu
// giữ số cũ trong ảnh chụp `fees` tới khi nhân viên mở ra bấm Lưu lại.
export const HCNS_PER_HEAD = 300000
...
// Trạng thái duyệt (price_status). Từ 2026-09-21 MỌI báo giá phải được quản trị duyệt mới xuất gửi khách:
//   ok       — đúng biểu phí, CHỜ DUYỆT
//   pending  — giá đề xuất khác biểu phí, CHỜ DUYỆT
//   approved — đã duyệt → xuất Word, gửi/ký HĐ, nộp file báo giá vào Drive
//   rejected — bị từ chối, nhân viên sửa rồi Lưu là quay về chờ duyệt
// (Tên 'ok' giữ lại vì dữ liệu cũ; 7 báo giá lập trước quy định này đã được chuyển sang 'approved'.)
// ⚙ CÔNG TẮC "mọi báo giá phải quản trị duyệt" — TẠM TẮT từ 2026-09-21 theo yêu cầu người dùng ("sẽ bật lại
// sau"). Tắt: báo giá đúng biểu phí xuất bình thường, CHỈ báo giá đề xuất mức khác biểu phí phải chờ duyệt
// (nguyên tắc kiểm soát giá mục 4.7 tài liệu bàn giao, như trước 2026-09-21). Bật lại = đổi thành true,
// và chạy scripts/approve-existing-sales-quotes.mjs --apply TRƯỚC khi deploy để báo giá cũ không bị khoá.
export const APPROVE_ALL_QUOTES = false

export const PRICE_STATES = {
  // sub: dòng phụ in nhỏ bên dưới (nhãn 2 dòng cho cột "Lưu ý" hẹp lại, bảng không tràn ngang)
  ok: APPROVE_ALL_QUOTES
    ? { label: 'Chờ duyệt', sub: 'đúng biểu phí', cls: 'bg-amber-50 text-amber-800 border-amber-200' }
    : { label: 'Đúng biểu phí', cls: 'bg-emerald-50 text-emerald-700 border-emerald-200' },
  pending:  { label: 'Chờ duyệt', sub: 'giá đề xuất', cls: 'bg-red-50 text-red-700 border-red-200' },
  approved: { label: 'Đã duyệt',   cls: 'bg-emerald-50 text-emerald-700 border-emerald-200' },
  rejected: { label: 'Bị từ chối', cls: 'bg-gray-100 text-gray-600 border-gray-200' },
}
export const awaitingApproval = status => status === 'pending' || (APPROVE_ALL_QUOTES && status === 'ok')
// Được xuất Word gửi khách / chuyển Gửi hợp đồng, Chốt hợp đồng.
export const canExportPrice = status => status === 'approved' || (!APPROVE_ALL_QUOTES && status === 'ok')

// Trạng thái duyệt sau khi nhân viên LƯU LẠI một báo giá đã có (chốt 2026-09-21):
// đã duyệt mà phí + số liệu tính phí không đổi (chỉ sửa địa chỉ, SĐT, lưu ý...) → giữ đã duyệt;
// còn lại (đổi phí, đổi số liệu, hoặc đang bị từ chối/chờ duyệt) → quay về chờ duyệt.
export function nextPriceStatus(prev, { priceChanged, overrideOn }) {
  if (prev === 'approved' && !priceChanged) return 'approved'
  return overrideOn ? 'pending' : 'ok'
}

// Phí/số liệu tính phí có đổi không — so ảnh chụp phí, phí thực và đề xuất giá.
export function pricingChanged(saved, next) {
  return JSON.stringify(saved.fees || {}) !== JSON.stringify(next.fees || {}) ||
    Number(saved.monthly_final) !== Number(next.monthlyFinal) ||
    !!saved.override_on !== !!next.overrideOn ||
    (next.overrideOn && (Number(saved.override_amount) !== Number(next.overrideAmount) ||
      String(saved.override_reason || '') !== String(next.overrideReason || '')))
}
