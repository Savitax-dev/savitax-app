...
// Tải nội dung 1 file về (Uint8Array) — dùng để xem lại phiếu khảo sát khách đã nộp.
export async function downloadFile(fileId) {
  const token = await accessToken()
  const r = await fetch(API + '/files/' + fileId + '?alt=media&' + ALL, { headers: { Authorization: 'Bearer ' + token } })
  if (!r.ok) throw new Error('Không tải được file từ Drive: ' + r.status)
